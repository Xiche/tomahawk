...
IdleCB *CreateIdleCallback(Callback * proc, void *userData, Callback * freeProc)
{
     ...
     */
    for (prev = NULL, curr = idleCallbacks; curr != NULL; prev = curr, curr = curr->next)
    {
    }
...
TimerCB *CreateTimerCallback(int milliseconds, Callback * proc, void *userData, Callback * freeProc)
{
    ...
    {
        fprintf(stderr, "Ran out of space for timers.  Recompile" "with larger value of MAX_TIMERS\n");
        exit(1);
    }
...
void DeleteFileCallback(FileCB * cb)
{
    ...
    for (prev = NULL, curr = fileCallbacks; curr != NULL; prev = curr, curr = curr->next)
    {
        if (cb == curr)
        {
            break;
 ...
 *----------------------------------------------------------------------
 */
int DoOneEvent(int block)       /* (IN) Should we block waiting on an event? */
{
    static int numFds = 0;
    int maxFd;
...
int HostToAddr(struct sockaddr_in *addr,    /* (OUT) Parsed address */
...
void SendQualifiedCmd(Peer * p, int id, char type, char *str)
{
    int len = strlen(str);
    char cmdHdr[32];
...
static void ProcessCmd(Peer * p, char *buffer, int len)
{
    CmdHandler *ch;
...
static void RecvCmd(void *data)
{
    ...
    if (p->cmdLen != 0)
    {
         ...
         * it and return.
         */
        numRead = read(p->ctrl, p->cmdBuffer + p->numRecv, p->cmdLen - p->numRecv);
        if (numRead == 0)
        {
            CloseConnection(p);
            return;
    ...
    assert(p->cmdType == 'x' || p->cmdType == 'd' || p->cmdType == 'e' || p->cmdType == 'r');
    endPtr += 2;
    assert(*endPtr == ' ');
    while (*endPtr == ' ')
    {
...
static void AcceptConnection(void *data)
{
...
void CloseConnection(Peer * p)
{
    ...
    if (p->server != NULL)
    {
        for (prev = NULL, curr = p->server->p; curr != NULL; prev = curr, curr = curr->next)
        {
            if (curr == p)
            {
                if (prev != NULL)
...
void DeleteCmdHandler(CmdHandler * cb)
{
    ...
    for (prev = NULL, curr = cmdHandlers; curr != NULL; prev = curr, curr = curr->next)
    {
        if (cb == curr)
        {
            break;
...
int ConnectToServer(Peer * p, char *hostname, int port)
{
    ...
    {
        fprintf(stderr, "Error connecting to host %s: %s", hostname, strerror(errno));
        return 0;
    }
