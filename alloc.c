...
static void ValidateMemory(struct mem_header *memHeaderP, char *file, int line, int nukeGuards);
...
static void ValidateMemory(memHeaderP, file, line, nukeGuards)
                                 ...
                                 * after they have been printed */
{
    ...
    for (idx = 0; idx < LOW_GUARD_SIZE; idx++)
    {
        ...
        if (byte != GUARD_VALUE)
        {
            ...
            fflush(stdout);
            byte &= 0xff;
            fprintf(stderr, "low guard byte %d is 0x%x  \t%c\n", idx, byte, (isprint(UCHAR(byte)) ? byte : ' '));   /* INTL: bytes */
        }
    }
    if (guard_failed)
    {
...
void *DbCkalloc(unsigned int size, char *file, int line)
{
    ...
    result = (struct mem_header *) malloc((unsigned) size + sizeof(struct mem_header) + HIGH_GUARD_SIZE);
    if (result == NULL)
    {
        fflush(stdout);
        DumpMemoryInfo(stderr);
        fprintf(stderr, "unable to alloc %d bytes, %s line %d", size, file, line);
        exit(1);
    }
    ...
    {
        memset((void *) result, GUARD_VALUE, size + sizeof(struct mem_header) + HIGH_GUARD_SIZE);
    }
    else
    {
...
void Preserve(void *clientData)
{
    ...
    if (inUse == spaceAvl)
    {
        if (spaceAvl == 0)
        {
            refArray = (Reference *) malloc((unsigned) (INITIAL_SIZE * sizeof(Reference)));
        ...
        else
        {
            ...
            new = (Reference *) malloc((unsigned) (2 * spaceAvl * sizeof(Reference)));
...
void EventuallyFree(clientData, freeProc)
